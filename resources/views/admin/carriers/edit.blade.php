                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.carriers.update", [$carrier->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('carrier_name') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="carrier_name" id="carrier_name" value="{{ old('carrier_name', $carrier->carrier_name) }}" required>
                            @if($errors->has('carrier_name'))
                                <span class="help-block" role="alert">{{ $errors->first('carrier_name') }}</span>
                            @endif
                        ...
                        <div class="form-group {{ $errors->has('status') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control" name="status" id="status" required>
                                ...
                                @foreach(App\Models\Carrier::STATUS_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('status', $carrier->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('tracking_url') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="tracking_url" id="tracking_url" value="{{ old('tracking_url', $carrier->tracking_url) }}" required>
                            @if($errors->has('tracking_url'))
                                <span class="help-block" role="alert">{{ $errors->first('tracking_url') }}</span>
                            @endif
                        ...
                        <div class="form-group {{ $errors->has('phone') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="phone" id="phone" value="{{ old('phone', $carrier->phone) }}" required>
                            @if($errors->has('phone'))
                        ...
                        <div class="form-group {{ $errors->has('email') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="email" name="email" id="email" value="{{ old('email', $carrier->email) }}" required>
                            @if($errors->has('email'))
                        ...
                        <div class="form-group {{ $errors->has('brand_logo') ? 'has-error' : '' }}">
                            ...
                            @if($errors->has('brand_logo'))
                                <span class="help-block" role="alert">{{ $errors->first('brand_logo') }}</span>
                            @endif
...
Dropzone.options.brandLogoDropzone = {
    url: '{{ route('admin.carriers.storeMedia') }}',
    maxFilesize: 20, // MB
