...
<div class="content">
    ...
    <select class="form-control select2" name="user_id" id="user_id" required>
        @foreach($users as $id => $name)
            <option value="{{ $id }}" {{ (old('user_id', $expense->user_id) == $id) ? 'selected' : '' }}>{{ $name }}</option>
        @endforeach
    </select>
    ...
    @endif
                        ...
                        <div class="form-group {{ $errors->has('description') ? 'has-error' : '' }}">
                         ...
                         <div class="form-group {{ $errors->has('status') ? 'has-error' : '' }}">
                            <label for="status">{{ trans('cruds.expense.fields.status') }}</label>
                            <select class="form-control" name="status" id="status">
                                <option value="pending" {{ old('status', $expense->status) == 'pending' ? 'selected' : '' }}>Pending</option>
                                <option value="approve" {{ old('status', $expense->status) == 'approve' ? 'selected' : '' }}>Approve</option>
                                <option value="reject" {{ old('status', $expense->status) == 'reject' ? 'selected' : '' }}>Reject</option>
                            </select>
