...
<div class="content">
    @can('shop_setting_create')
        <div style="margin-bottom: 10px;" class="row">
            <div class="col-lg-12">
                <a class="btn btn-success" href="{{ route('admin.shop-settings.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $shopSetting->id }}">
                                        ...
                                        <td>
                                            @can('shop_setting_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('admin.shop-settings.show', $shopSetting->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('shop_setting_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('admin.shop-settings.edit', $shopSetting->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('shop_setting_delete')
                                                <form action="{{ route('admin.shop-settings.destroy', $shopSetting->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('shop_setting_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.shop-settings.massDestroy') }}",
