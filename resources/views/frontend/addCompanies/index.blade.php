        ...
        <div class="col-md-12">
            @can('add_company_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.add-companies.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $addCompany->id }}">
                                        ...
                                        <td>
                                            @can('add_company_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.add-companies.show', $addCompany->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('add_company_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.add-companies.edit', $addCompany->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('add_company_delete')
                                                <form action="{{ route('frontend.add-companies.destroy', $addCompany->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('add_company_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.add-companies.massDestroy') }}",
