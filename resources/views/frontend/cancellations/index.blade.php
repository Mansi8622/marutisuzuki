...
<div class="content">
    @can('cancellation_create')
        <div style="margin-bottom: 10px;" class="row">
            <div class="col-lg-12">
                <a class="btn btn-success" href="{{ route('frontend.cancellations.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $cancellation->id }}">
                                        ...
                                        <td>
                                            @can('cancellation_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.cancellations.show', $cancellation->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('cancellation_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.cancellations.edit', $cancellation->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('cancellation_delete')
                                                <form action="{{ route('frontend.cancellations.destroy', $cancellation->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('cancellation_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.cancellations.massDestroy') }}",
