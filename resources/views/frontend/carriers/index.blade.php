        ...
        <div class="col-md-12">
            @can('carrier_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.carriers.create') }}">
                            ...
                            <tbody>
                                @foreach($carriers as $key => $carrier)
                                    <tr data-entry-id="{{ $carrier->id }}">
                                        ...
                                        <td>
                                            @can('carrier_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.carriers.show', $carrier->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('carrier_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.carriers.edit', $carrier->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('carrier_delete')
                                                <form action="{{ route('frontend.carriers.destroy', $carrier->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('carrier_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.carriers.massDestroy') }}",
