    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.stock-transfers.store") }}" enctype="multipart/form-data">
                        @method('POST')
                        @csrf
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="select_products[]" id="select_products" multiple required>
                                @foreach($select_products as $id => $select_product)
                                    <option value="{{ $id }}" {{ in_array($id, old('select_products', [])) ? 'selected' : '' }}>{{ $select_product }}</option>
                                @endforeach
                            </select>
